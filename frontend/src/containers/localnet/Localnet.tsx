...
const Localnet: React.FC = () => {
  ...
  const [dispenseAmount, setDispenseAmount] = useState<number>(10);
  ...
  const transferAlgorand = async () => {
    if (!selectedWallet || !receiverAddress || dispenseAmount <= 0) {
      setTransactionError("Please fill in all fields correctly.");
      return;
    }
    ...
    try {
      ...
      // Handle response, possibly update wallets
      if (response.status === 200) {
        alert(`Transaction Successful: ${response.data.message}`);
        fetchWallets(); // Refresh wallet balances
        setShowDispenseModal(false); // Close modal after successful transaction
        setTransactionError(null); // Clear any previous error
      }
    } catch (err) {
      setTransactionError('Failed to send Algorand transaction');
      console.error(err);
    }
  };
  ...
  return (
    <div className="container mt-5">
      ...
      <Modal show={showTransactModal} onHide={() => setShowTransactModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Send Algorand</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group controlId="dispenseAmount">
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowTransactModal(false)}>
            Close
          </Button>
          <Button variant="primary" onClick={transferAlgorand}>
            Send
          </Button>
        </Modal.Footer>
      </Modal>
